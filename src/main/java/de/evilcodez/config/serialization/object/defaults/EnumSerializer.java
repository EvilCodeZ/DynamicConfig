...
public class EnumSerializer implements TypeSerializer<Enum> {
    ...
    @Override
    public BaseValue serialize(ObjectSerializer serializer, Enum value, Class<?> typeClass) {
        return new StringValue(value.name());
    }

    @Override
    public Enum deserialize(ObjectSerializer serializer, BaseValue value, Class<?> typeClass) {
